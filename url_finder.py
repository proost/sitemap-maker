...
class UrlCollecter(QThread):

    FINISHED = pyqtSignal(tuple)
    PROGRESS = pyqtSignal(float)
    DUMP = pyqtSignal(tuple)
    ...
    def run(self):
        ...
        for url in self.url_list_in_each_site:
            ...
            self.retrench_memory(url)
            self.PROGRESS.emit(0.03)
        self.FINISHED.emit((self.main_domain,self.url_list_in_each_site))
    ...
    def retrench_memory(self,url):
        self.url_list_in_each_site.remove(url)
        self.temporary_url_list.append(url)
        if len(self.temporary_url_list) > 100:
            self.DUMP.emit((self.main_domain,self.temporary_url_list))
            self.temporary_url_list.clear()
